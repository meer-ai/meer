...
/** One file-system match produced by a {@link FuzzyFileFinder}. */
export interface FuzzyFileEntry {
	path: string;
	isDirectory: boolean;
}

/**
 * Pluggable fuzzy file-search backend for the `@` picker. Injected by the host
 * so this package stays free of any concrete file-finding implementation; the
 * meer app wires its fff → fd → JS-walk cascade in. When no finder is provided
 * the `@` picker simply yields no file suggestions.
 */
export type FuzzyFileFinder = (opts: {
	basePath: string;
	query: string;
	maxResults?: number;
	signal?: AbortSignal;
}) => Promise<FuzzyFileEntry[]>;
...
export class CombinedAutocompleteProvider implements AutocompleteProvider {
	...
	private fileFinder: FuzzyFileFinder | null;

	constructor(
		commands: (SlashCommand | AutocompleteItem)[] = [],
		basePath: string,
		fdPath: string | null = null,
		fileFinder: FuzzyFileFinder | null = null,
	) {
	...
	private async getFuzzyFileSuggestions(
		...
		if (options.signal.aborted || !this.fileFinder) {
			return [];
		}
		...
		try {
			...
			const entries = await this.fileFinder({ basePath: fdBaseDir, query: fdQuery, maxResults: 100, signal: options.signal });
